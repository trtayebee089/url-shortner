...
const props = defineProps<{ link?: LinkRecord; submitLabel?: string; loading?: boolean; simpleCreate?: boolean; modal?: boolean }>()
const emit = defineEmits<{ submit: [payload: Record<string, unknown>]; change: [payload: Record<string, string | boolean>]; cancel: [] }>()
...
<template>
  <form class="grid" :class="modal ? 'gap-4' : 'surface gap-5 p-5 sm:p-6'" @submit.prevent="submit">
    ...
    <div class="flex items-center justify-end gap-2 border-t border-border pt-4"><button v-if="modal" type="button" class="btn-secondary" @click="emit('cancel')">Cancel</button><button class="btn-primary" :disabled="loading"><span v-if="loading">Creating…</span><span v-else>{{ submitLabel || 'Save link' }}</span></button></div>
  </form>
</template>
